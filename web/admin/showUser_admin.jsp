...
<c:if test="${empty allpage}">
    <c:redirect url="/admin/ShowUserServlet"></c:redirect>
</c:if>
<%
    Integer curren = request.getParameter("pagel")==null?1:Integer.valueOf(request.getParameter("pagel"));
    Object allpage1 = request.getAttribute("allpage");
    Boolean []admins = (Boolean[]) request.getAttribute("admins");
    Integer allpage = 1;
    if(allpage1 !=null)
        allpage = Integer.valueOf(allpage1.toString());
%>
<head>
    <meta charset="utf-8">
    ...
    <script type="text/javascript">
        ...
        function admin(id){
            ...
            var url ="submitAdminServlet?id="+id;
    ...
    <div class="main">
        ...
        <c:forEach var="user" items= "${users}">
            <tr>
                ...
                <td>${user.pswd}</td>
                <td>
                        <input onclick="admin(${user.id});" type="checkbox" <%=admins[index++]?"checked":""%>  name = "${user.id}">
                </td>
            </tr>
        </c:forEach>
        </table>
        <a href="ShowUserServlet?pagel=<%=curren-1<1?1:(curren-1)%>">last</a>
        <a><%=curren%>/</a>
        <a><%=allpage%></a>
        <a href="ShowUserServlet?pagel=<%=curren+1>allpage?allpage:curren+1%>">next</a>
